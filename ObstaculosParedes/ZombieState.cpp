...
void State_Zombie_Deambular::Enter(Zombie* zombie){
...
void State_Zombie_Deambular::Execute(Zombie* zombie){
...
void State_Zombie_Deambular::Exit(Zombie* zombie){

	zombie->GetSteeringBehaviors().EvadirParedesOff();
	zombie->GetSteeringBehaviors().m_PesoEvadirParedes = 2.0f;// El valor por defecto es 1.0f, este escala el resultado
	zombie->GetSteeringBehaviors().EvadirObstaculosOff();		 // de la fuerza de EvadirParedes
	zombie->GetSteeringBehaviors().m_PesoEvadirObstaculos = 2.0f;// El valor por defecto es 1.0f, este escala el resultado
	zombie->GetSteeringBehaviors().DeambularOff();
}

char* State_Zombie_Deambular::GetStateName(){
	return "Deambular";
}
